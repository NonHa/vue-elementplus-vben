<template>
  ...
  >
    <template #form-advanceBefore>
      <el-button type="danger" @click="toggleCanResize({}, 3)">选择品牌</el-button>
    </template>
...
let columns = getHomeBrandColumns();
...
let sureEditForm = async (type) => {
  ...
  if (unref(editRow).id) {
    ...
    await updateHomeBrand(item).then((res) => {
      if (res) {
        unref(modalRef).visibleRef = false;
        reload();
        return true;
...
function handelDelete(row) {
  deleteHomeBrand({ id: row.id }).then((res) => {
    if (res.code === 200) {
      reload();
    }
  });
